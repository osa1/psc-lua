...
untilFixpoint f a =
    let a' = f a
    in if a' == a then a else untilFixpoint f a'
...
optimize = applyAll
  [applyVars', applyVars, inlineEffOps, removeReturnApps, removeVarAsgns,
      ...
      (L.Block stats (Just
        [ L.PrefixExp (L.PEFunCall
            (L.NormalFunCall
             ...
             (L.Args vars))) ]))
      | all isVar vars && not (null args) && length args == length vars =
